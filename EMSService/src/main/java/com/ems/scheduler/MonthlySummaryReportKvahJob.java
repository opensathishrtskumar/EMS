...
public class MonthlySummaryReportKvahJob extends AbstractJob {
	...
	protected void work(JobExecutionContext arg0) throws JobExecutionException {
		...
		File reportTemplate = new File(
				TempDataManager.getTempFolder().getAbsoluteFile() + File.separator + "MONTHLY_SUMMARYKvah.xlsx");
		File report = new File(reportPath + File.separator + this.emailDTO.getDate() + "-MonthlySummaryKvah.xlsx");
		...
		for (String device : consumptionDevices) {
			try {
				...
				if (!consumptionList.isEmpty()) {
					...
					String kwAddress = valueAndAddress.getProperty(VAH);
