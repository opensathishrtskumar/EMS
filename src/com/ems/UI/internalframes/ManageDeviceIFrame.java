...
public class ManageDeviceIFrame extends JInternalFrame {
	...
	private static final Logger logger = LoggerFactory
			.getLogger(ManageDeviceIFrame.class);
	...
	public ManageDeviceIFrame() {
		...
		panel_1.setBorder(new TitledBorder(new CompoundBorder(null, new EtchedBorder(EtchedBorder.LOWERED, null, null)), "Actions", TitledBorder.CENTER, TitledBorder.TOP, null, new Color(0, 0, 0)));
			...
			public void actionPerformed(ActionEvent arg0) {
				...
				if(table.getRowCount() <= defaultDevice){
					...
					JOptionPane.showMessageDialog(getMe(),
							"Application is configured to have only "
									+ defaultDevice + " Devices",
							"Manage Device", JOptionPane.WARNING_MESSAGE);
				}
			}
		});
		...
		btnSaveChanges.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				if(!dbConfig){
					JOptionPane.showMessageDialog(getMe(), "DB not configured", 
							"DB Configuration", JOptionPane.WARNING_MESSAGE);
					return;
				}
				...
				try {
					...
					JOptionPane.showMessageDialog(getMe(), "Saved successfully", "Success", JOptionPane.INFORMATION_MESSAGE);
				} catch (Exception e) {
					logger.error("{}",e);
					JOptionPane.showMessageDialog(getMe(), e.getLocalizedMessage(), "Failed", JOptionPane.ERROR_MESSAGE);
				}
			}
		});
		...
		EMSSwingUtils.addItemsComboBox(comboBoxPorts, 0,
				EMSUtility.getAvailablePort());
		...
		comboBoxPollDelay.setModel(new DefaultComboBoxModel<String>(
		...
		btnRemoveRow.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				if (selectedRow == -1 || table.getRowCount() == 1) {
					JOptionPane.showMessageDialog(getMe(),
							"Invalid Selection", "Remove device",
				...
				if (uniqueid.intValue() != 0) {
					JOptionPane.showMessageDialog(getMe(),
							"Cannot delete existing device, please disable it",
		...
		panel_2.setBorder(new TitledBorder(new CompoundBorder(null, new CompoundBorder(null, new EtchedBorder(EtchedBorder.LOWERED, null, null))), "Devices", TitledBorder.LEADING, TitledBorder.TOP, null, new Color(0, 0, 0)));
	...
	private void populateDBDeviceDetails(final JTable table){
		...
		if(availableDevices != null && !availableDevices.isEmpty()){
			for(DeviceDetailsDTO device : availableDevices){
				Object[] deviceRow = new Object[] { device.getUniqueId(), 
		...
		table.getModel().addTableModelListener(new TableModelListener() {
			...
			public void tableChanged(TableModelEvent event) {
				...
				if (event.getType() == event.INSERT) {
					logger.info("Tabel model changed INSERT, selected row {} ",
					...
					if (value instanceof MyComboModel) {
						logger.info("Selected value : {}",
								((MyComboModel) value).getSelectedItem());
					} else {
	...
	public int getComboBoxPollDelay() {
		int pollDelay = Integer.parseInt(comboBoxPollDelay.getSelectedItem()
				.toString());
		return pollDelay;
	}
