...
public class SummaryWorker implements Callable<Object> {
	...
	public Map<DeviceDetailsDTO, List<SummaryReportDTO>> call() throws Exception {
		...
		if (devices != null) {
			...
			DBConnectionManager.closeConnections(connection, null, null);
		}
	...
	private Map<String, ArrayList<Object>> extractResultSet(ResultSet rs, Map<String, String> headers) throws SQLException {
		...
		while (rs.next()) {
			...
			Properties unitResponse = EMSUtility.loadProperties(rs.getString("unitresponse"));
	...
	private ArrayList<SummaryReportDTO> calculateSummary(Map<String, ArrayList<Object>> responseArray, Map<String, String> headers){
	...
	private SummaryReportDTO calculateData(SummaryReportDTO summary, ArrayList<Object> array) {
		...
		//logger.trace(" min index : {} max index : {} min : {} max : {} sum : {} avg : {} ",minIndex,maxIndex,min,max,sum,average);
